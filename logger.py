...
class Logger:
    ...
    def save(self, config: Config, method: str) -> None:
        ...
        os.makedirs(f"{self.logdir}/{dataset}_{model}", exist_ok=True)
        with open(f"{self.logdir}/{dataset}_{model}/log_{method}_mutr{mut_rate_str}_mutv{mut_var_str}_tour{tour_size}_{seed}.csv", "w") as f:
        ...
        with open(f"{self.logdir}/{dataset}_{model}/result_{method}_mutr{mut_rate_str}_mutv{mut_var_str}_tour{tour_size}_{seed}.jsonl", "w") as f:
            json.dump(self.best_ind_data, f, indent=2)

        if self.ei_history:
            with open(f"{self.logdir}/{dataset}_{model}/ei_{method}_mutr{mut_rate_str}_mutv{mut_var_str}_tour{tour_size}_{seed}.csv", "w") as f:
    ...
    def save_tpe_params(self, config: Config, method: str, best_params: Dict) -> None:
        ...
        with open(f"{self.logdir}/{dataset}_{model}/result_{method}_mutr{mut_rate_str}_mutv{mut_var_str}_tour{tour_size}_{seed}.jsonl", "a") as f:
