...
class Logger:
    ...
    def log_best(self, population, config, method):
    ...
    def save(self, config, method):
        dataset = config.get_dataset_id()
        seed = config.get_seed()
        os.makedirs(f"{self.logdir}_data{dataset}", exist_ok=True)
        with open(f"{self.logdir}_data{dataset}/log_{method}_{seed}.csv", "w") as f:
            f.write("generation,best,average,median\n")
            for gen, best, avg, median in self.history:
                f.write(f"{gen},{best},{avg},{median}\n")

        with open(f"{self.logdir}_data{dataset}/result_{method}_{seed}.json", "w") as f:
