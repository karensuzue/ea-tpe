...
#   './results/mut-var/{TASK ID}/result_TPEBO_{RUN}.jsonl'
# Path structure (TPEC experiments): 
#   './results/mut-var/{TASK ID}/result_TPEC_mutr{MUT_RATE}_mutv{MUT_VAR}_tour{TOUR_SIZE}_{RUN}.jsonl'
...
def make_tpebo_df_per_task(results_path: str, task_id: int):
    ...
    filename_re_190410 = re.compile(
        r"result_TPEBO_mutr([0-9,\.]+)_mutv([0-9,\.]+)_tour([0-9]+)_([0-9]+)\.jsonl"
        )
    filename_re = re.compile(r"result_TPEBO_([0-9]+)")
    ...
    for filename in os.listdir(id_folder):
        ...
        # does filename match?
        if task_id == 190410: # special case - 190410 (I'm lazy)
            m = filename_re_190410.match(filename)
            if not m:
...
if __name__ == "__main__":
    ...
    # tasks = [359959]
    # mut_rates = [0.1]
    # mut_vars = [0.2, 0.05, 1.0]
    # tour_sizes = [2]

    tasks = [2073, 168757, 168784, 190146, 190410]
    mut_rates = [1.0]
    mut_vars = [0.25, 0.35, 0.4]
    tour_sizes = [5, 10, 25]

    results_path = '../../results/mut-var2'

    tpec_df = pd.DataFrame()
    for task in tasks:
        df = make_tpec_df_per_task(results_path, task, mut_rates, mut_vars, tour_sizes)
    ...
    merged_df.to_csv("mut-var2.csv", index=False)